...
export interface PlatformConfig {
  ...
  postLoginCheck?: string; // Element to wait for after login
  cookies?: string[]; // Key cookies that indicate successful login
  notes?: string;
}
...
export function getPlatform(query: string): PlatformConfig | null {
...
export function listPlatforms(category?: PlatformConfig['category']): PlatformConfig[] {
  if (!category) {
...
export function searchPlatforms(query: string): PlatformConfig[] {
  const lower = query.toLowerCase();
